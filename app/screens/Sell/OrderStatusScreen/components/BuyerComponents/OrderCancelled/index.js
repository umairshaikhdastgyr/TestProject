...
const OrderCancelled = ({ orderData, onViewReceipt, storeName }) => {
  ...
  const getShippingAddress = () => {
    const address = `${orderData?.deliveryMethod?.addressline1}${orderData?.deliveryMethod?.addressline2 &&
    ...
    <>
      <ScrollView>
        ...
        <View style={styles.cancelledMessageContainer}>
          <Text style={styles.cancelledMsgText}>
            {((orderData?.buyerId == orderData?.cancelRequest?.userId) && orderData?.order_status == "cancelled") ? `The seller has accepted your cancellation request. Your funds will
            be returned in the next 3 business day.` : "The seller cancelled this order. Your funds will be refunded in next 3 days."}
          </Text>
        </View>
