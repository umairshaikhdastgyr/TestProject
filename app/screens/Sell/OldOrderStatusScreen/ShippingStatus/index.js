...
const ShippingStatus = ({ order, cardTitle, cardDetail, type, side, orderstatus }) => {
  const {
    deliveryStatus,
    paymentMethod,
    deliveryMethod,
  ...
  const trackingCarrier = !type && order?.labels && order?.labels.length && order?.labels[order?.labels.length - 1].carrier;
  const trackingId = !type && order?.labels && order?.labels.length && order?.labels[order?.labels.length - 1].trackingId;
  const carrierName = !type && order?.labels && order?.labels.length && order?.labels[order?.labels.length - 1].carrier;
  ...
  const getShippingAddress = () => {
    const address = `${deliveryMethod?.addressline1}${deliveryMethod?.addressline2 && ", " + deliveryMethod?.addressline2
      }\n${deliveryMethod?.city && "" + deliveryMethod?.city}${deliveryMethod?.state && ", " + deliveryMethod?.state
    ...
    <View style={styles.container}>
      <View style={styles.cardContainer}>
        <StatusCard
          ...
          header={
            type == "return"
              ? !shippedActive
                ? side == 'BUYER' ? "Return by" : 'Buyer Ship By'
                : "Shipped on"
              : shippedAt
                ? "Shipped on"
                : "Ship By"
      ...
      {claimTrackingId && type == "claim" && (
        <View style={styles.trackClaimContatiner}>
          ...
          {orderstatus != 'claimclosed' && orderstatus != 'claimdisputed' && side == 'BUYER' ? <Text
            style={[
              styles.trackText,
              { textAlign: "center", color: "#313334", marginTop: 30 },
            ]}
          >{`You’ll see an update once the seller receives the item. Once they receive it, you’ll be issued your refund.`}</Text> : null}
        </View>
      )}
      {trackingId &&
      ...
      {(ReturnRequests?.[0]?.returnStatus?.toLowerCase() == "labelshared" ||
        ReturnRequests?.[0]?.returnStatus?.toLowerCase() ==
        "labelsharedind") && side == 'BUYER' && (
          <Text
            style={[
              styles.trackText,
              { textAlign: "center", color: "#313334", marginTop: 30 },
      ...
      {(ReturnRequests?.[0]?.returnStatus == "returnshipped" ||
        ReturnRequests?.[0]?.returnStatus == "returned") && side == 'BUYER' && (
          <>
            <Text
              style={[
                styles.trackText,
            ...
            >{`${ReturnRequests?.[0]?.labelData?.data?.carrier?.toUpperCase() || ReturnRequests?.[0]?.labelData?.carrier?.toUpperCase()} ${ReturnRequests?.[0]?.labelData?.data?.trackingId || ReturnRequests?.[0]?.labelData?.trackingId?.toUpperCase()}`}</Text>
            <Text
              style={[
                styles.trackText,
                { textAlign: "center", color: "#313334", marginTop: 30 },
              ]}
            >{`You’ll see an update once the seller receives the item. Once they receive it, you’ll be issued your refund.`}</Text>
          </>
        )}
      {side === 'BUYER' && type != "return" &&
        type != "claim" &&
      ...
      {side === 'BUYER' && type != "return" && type != "claim" && paymentMethodText && (
        <BorderElement
          txtType="bold"
