...
const HeaderCatalog = ({
  ...
  const openShareItemOptions = async () => {
    const link = await getFirebaseLink(`?catalogId=${catalogData.id}`);

    const shareOptions = {
      title: "Share Item",
      message: `Checkout ${catalogData?.name} in Homitag ${link}`,
    };
  ...
  return (
    <View style={[styles.header, { height: 60 }]}>
      ...
      >
        ...
        <Text style={styles.headerText}>{catalogName}</Text>
